...
char *get_animal_type_name(enum animal_type type)
{
	...
	switch (type) {
	case CAT:
		result = "Красный чемодан";
		break;
	case DOG:
		result = "Серый чемодан";
		break;
	case COW:
		result = "Коричневый чемодан";
		break;
	case PIG:
		result = "Синий чемодан";
		break;
	default:
		result = "N/A";
	}
...
void show_animals(struct animal animals[], unsigned int count)
{
	for (unsigned int i = 0; i < count; i++) {
		printf("Информация про багаж №%02u: ", i + 1);
		printf("%s: высота = %u см, вес = %u кг. \n",
